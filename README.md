# 『물류데이터 사이언스와 ChatGPT 활용』 

- 본 도서는 중앙대학교 글로벌인적자원개발대학원 글로벌물류학과의 '물류데이터 사이언스와 딥러닝'의 교재이다.
  
- SCM/물류 데이터분석 역량 강화를 위한 『ChatGPT 실무적용 SCM/물류 데이터분석』 (연세대학교 출판문화원)의 후속 도서이다.
  
- 본 사이트에는 『물류데이터 사이언스와 ChatGPT 활용』의 본문에서 언급한
  
  기준정보, ChatGPT 요청문, 파이썬코드 등이 장별로 구분되어 저장되어 있다(단, 14장, 15장, 16장은 업로드한 파일 없음).

- 전체 실습 데이터 즉, 각 장의 압축 파일은 1장 SCM물류 데이터분석 역량 폴더에 모두 저장되어있다.  
 
- 한국생산성본부에서'ChatGPT 활용 SCM 재고 최적화 전략', '공급망관리(SCM) 운영모델 혁신(ChatGPT 활용)'과 "ChatPG 활용 물류 운영 최적화" 과정'을 통해
  『ChatGPT 실무적용 SCM/물류 데이터분석』와 『물류데이터 사이언스와 ChatGPT 활용』의 일부 내용이 저자 직강으로 운영되고 있다.
   - 아래 밑줄 클릭하여 한국생산성본부 사이트 접속하여 상세 커리큐럼을 확인하기 바랍니다(강력 추천, 강의 수강 시  『ChatGPT 실무적용 SCM/물류 데이터분석』도서도 제공합니다.)
   - [한국생산성본부_ChatGPT 활용 SCM 재고 최적화 전략](https://kpc.or.kr/PTWED003_dtil_view.do?ecno=47833) (2일 과정)
   - [한국생산성본부_공급망관리(SCM) 운영모델 혁신(ChatGPT 활용)](https://www.kpc.or.kr/PTWED003_dtil_view.do?ecno=47835) (3일 과정)
   - [한국생산성본부_ChatGPT활용 물류 운영 최적화](https://kpc.or.kr/PTWED003_dtil_view.do?ecno=47840) (2일 과정)
 
- 본 도서의 일부 오타 내용은 "정오표_물류데이터 사이언스와 ChatGPT 활용_20260101.pdf"을 참조한다.

